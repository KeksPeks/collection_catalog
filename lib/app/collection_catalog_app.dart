...
class _CollectionShellState extends State<_CollectionShell> {
  ...
  Widget build(BuildContext context) {
    ...
    if (useRail) {
      return Scaffold(
        body: Row(children: [
          NavigationRail(selectedIndex: _index, onDestinationSelected: (value) => setState(() => _index = value), labelType: NavigationRailLabelType.all, groupAlignment: -0.85, destinations: [for (final destination in destinations) NavigationRailDestination(icon: Icon(destination.icon), selectedIcon: Icon(destination.selectedIcon), label: Text(destination.label))]),
    ...
    return Scaffold(
      ...
      bottomNavigationBar: NavigationBar(selectedIndex: _index, onDestinationSelected: (value) => setState(() => _index = value), destinations: [for (final destination in destinations) NavigationDestination(icon: Icon(destination.icon), selectedIcon: Icon(destination.selectedIcon), label: destination.label)]),
    );
  }
}
...
class _SettingsPage extends StatelessWidget {
  ...
  Future<void> _showInterfaceSettings(BuildContext context) async {
    ...
    await showModalBottomSheet<void>(
      ...
      builder: (sheetContext) => StatefulBuilder(builder: (context, setSheetState) => SafeArea(child: ListView(shrinkWrap: true, padding: const EdgeInsets.all(16), children: [
        const Text('Настройки интерфейса', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w800)),
        const SizedBox(height: 14),
        DropdownButtonFormField<int>(initialValue: columns, decoration: const InputDecoration(labelText: 'Колонки', border: OutlineInputBorder()), items: const [DropdownMenuItem(value: 0, child: Text('Авто')), DropdownMenuItem(value: 1, child: Text('1')), DropdownMenuItem(value: 2, child: Text('2')), DropdownMenuItem(value: 3, child: Text('3')), DropdownMenuItem(value: 4, child: Text('4'))], onChanged: (value) => setSheetState(() => columns = value ?? 0)),
        const SizedBox(height: 12),
        DropdownButtonFormField<String>(initialValue: density, decoration: const InputDecoration(labelText: 'Плотность', border: OutlineInputBorder()), items: const [DropdownMenuItem(value: 'auto', child: Text('Авто')), DropdownMenuItem(value: 'compact', child: Text('Компактная')), DropdownMenuItem(value: 'normal', child: Text('Обычная')), DropdownMenuItem(value: 'spacious', child: Text('Просторная'))], onChanged: (value) => setSheetState(() => density = value ?? 'auto')),
        const SizedBox(height: 12),
        DropdownButtonFormField<String>(initialValue: navigation, decoration: const InputDecoration(labelText: 'Навигация', border: OutlineInputBorder()), items: const [DropdownMenuItem(value: 'auto', child: Text('Авто')), DropdownMenuItem(value: 'bottom', child: Text('Нижняя панель')), DropdownMenuItem(value: 'side', child: Text('Боковая панель'))], onChanged: (value) => setSheetState(() => navigation = value ?? 'auto')),
        const SizedBox(height: 16),
        FilledButton(onPressed: () async { await UiLayoutSettings.save(columns: columns, density: density, navigation: navigation); if (sheetContext.mounted) Navigator.pop(sheetContext); }, child: const Text('Сохранить')),
      ]))),
    );
  }
...
class _SettingsGroup extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Card(clipBehavior: Clip.antiAlias, child: Padding(padding: const EdgeInsets.symmetric(vertical: 8), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Padding(padding: const EdgeInsets.fromLTRB(16, 8, 16, 4), child: Text(title, style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w800))), ...children]));
  }
}
