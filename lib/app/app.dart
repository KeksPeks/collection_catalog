...
class _BottomNavigationBar extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Material(color: Theme.of(context).colorScheme.surface, elevation: 3, child: SafeArea(top: false, child: SizedBox(height: 76, child: Row(children: [for (var index = 0; index < destinations.length; index++) Expanded(child: _BottomNavigationItem(data: destinations[index], selected: index == selectedIndex, onTap: () => onSelected(index)))]))));
}
...
class _SettingsPage extends StatelessWidget {
  ...
  Future<void> _showCatalogVersions(BuildContext context) async {
    await showModalBottomSheet<void>(context: context, isScrollControlled: true, builder: (sheetContext) => DraggableScrollableSheet(expand: false, initialChildSize: 0.65, maxChildSize: 0.92, builder: (_, controller) => SafeArea(child: ListView.builder(controller: controller, itemCount: CatalogRegistry.all.length + 1, itemBuilder: (_, index) {
