  ...
  Widget build(BuildContext context) => Material(
        ...
        child: SafeArea(
          ...
          child: SizedBox(
            height: 76,
            child: Row(
              children: [for (var index = 0; index < destinations.length; index++) Expanded(child: _BottomNavigationItem(data: destinations[index], selected: index == selectedIndex, onTap: () => onSelected(index)))],
            ),
          ),
        ),
      );
...
class _SettingsPage extends StatelessWidget {
  ...
  String _colorName(BuildContext context, int index) => [
        _text(context, 'Индиго', 'Indigo'),
        _text(context, 'Бирюзовый', 'Teal'),
        _text(context, 'Фиолетовый', 'Purple'),
        _text(context, 'Оранжевый', 'Orange'),
        _text(context, 'Зелёный', 'Green'),
      ][index];
      ...
      body: ListView(
        ...
        children: [
          ...
          _SettingsGroup(title: _text(context, 'Дополнительно', 'More'), children: [
            ...
            ListTile(leading: const Icon(Icons.help_outline), title: Text(_text(context, 'О приложении', 'About')), subtitle: Text(_text(context, 'Collection Catalog', 'Collection Catalog')), onTap: () => showAboutDialog(context: context, applicationName: 'Collection Catalog')),
          ]),
          if (text > 1.25) const SizedBox(height: 8),
        ],
      ),
    );
  }
  ...
  Future<void> _showColumns(BuildContext context) async {
    final selected = await showModalBottomSheet<int>(
      ...
      builder: (sheetContext) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [
        for (final value in [0, 1, 2, 3, 4]) ListTile(
          leading: Icon(UiLayoutSettings.columns == value ? Icons.radio_button_checked : Icons.radio_button_unchecked),
          title: Text(value == 0 ? _text(context, 'Автоматически', 'Automatic') : '$value'),
          subtitle: value == 0 ? Text(_text(context, 'Адаптация под ширину экрана', 'Adapt to screen width')) : null,
      ...
      builder: (sheetContext) => DraggableScrollableSheet(
        ...
        builder: (_, controller) => SafeArea(
          child: ListView.builder(
            ...
            itemCount: CatalogRegistry.all.length + 1,
            itemBuilder: (_, index) {
              ...
              return FutureBuilder<int?>(
                future: CatalogVersionStore.installedVersion(catalog.id),
                builder: (_, snapshot) => ListTile(
                  leading: const Icon(Icons.inventory_2_outlined),
                  title: Text(catalog.name),
                  subtitle: Text(snapshot.data == null ? _text(context, 'Не загружен', 'Not installed') : '${_text(context, 'Установлена', 'Installed')}: ${snapshot.data} · ${_text(context, 'Доступна', 'Available')}: ${catalog.version}'),
                ),
              );
            },
          ),
  ...
  Future<void> _exportBackup(BuildContext context) async {
    try {
      final snapshot = <String, dynamic>{
        ...
        'itemStates': await ItemStateStore.loadAll().then((items) => items.map((key, value) => MapEntry(key, value.toJson()))),
        'favorites': (await FavoritesStore.loadKeys()).toList(),
        'history': (await CollectionHistoryStore.load()).map((entry) => entry.toJson()).toList(),
      };
