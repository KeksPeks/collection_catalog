...
class CollectionHistoryStore {
  ...
  static Future<List<CollectionHistoryEntry>> load() async {
    ...
    return decoded.whereType<Map>().map((item) => CollectionHistoryEntry.fromJson(Map<String, dynamic>.from(item))).toList(growable: false);
  }

  static Future<void> record({required String type, required String title, required String details}) async {
    final entries = (await load()).toList();
    entries.insert(0, CollectionHistoryEntry(type: type, title: title, details: details, timestamp: DateTime.now()));
    if (entries.length > _maxEntries) entries.removeRange(_maxEntries, entries.length);
    final preferences = await SharedPreferences.getInstance();
    await preferences.setString(_key, jsonEncode(entries.map((item) => item.toJson()).toList()));
  }
