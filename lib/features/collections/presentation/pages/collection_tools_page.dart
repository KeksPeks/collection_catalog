...
class _CollectionToolsPageState extends ConsumerState<CollectionToolsPage> {
  ...
  Future<void> _load() async {
    ...
    final historyFuture = CollectionHistoryStore.load();
    final statesFuture = ItemStateStore.loadAll();
    final favoritesFuture = FavoritesStore.loadKeys();
  ...
  Map<String, dynamic> _snapshot() => {
        ...
        'catalogVersions': {for (final c in CatalogRegistry.all) c.id: c.version},
        'collections': [
          for (final s in _summaries)
            {
              'id': s.collection.id,
              'name': s.collection.name,
              'templateId': s.collection.templateId,
              'total': s.total,
              'owned': s.owned,
              'wanted': s.wanted,
              'ordered': s.ordered,
        ...
        'itemStates': _states.map((k, v) => MapEntry(k, v.toJson())),
  ...
  List<List<dynamic>> _rows() => [
        ...
        ..._summaries.map((s) => [s.collection.name, s.total, s.owned, s.wanted, s.ordered, s.quantity, s.cost.toStringAsFixed(2)]),
      ];
  ...
  Future<void> _export(String type) async {
    try {
      ...
      if (type == 'csv') await BackupExportService.exportCsv(_rows());
      if (type == 'xlsx') await BackupExportService.exportExcel(_rows());
      if (type == 'pdf') await BackupExportService.exportPdf(_rows());
      if (type == 'zip') await BackupExportService.exportZip(snapshot);
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Экспорт сформирован.')));
    } catch (error) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Ошибка экспорта: $error')));
    }
  }
  ...
  Future<void> _versions() async {
    final rows = <Widget>[];
    for (final c in CatalogRegistry.all) {
      final installed = await CatalogVersionStore.installedVersion(c.id);
      rows.add(
        ListTile(
          leading: const Icon(Icons.new_releases_outlined),
          title: Text(c.name),
          subtitle: Text('Опубликовано v${c.version} · На устройстве ${installed ?? '—'}'),
          trailing: installed != null && installed < c.version ? const Chip(label: Text('Обновить')) : null,
        ),
      );
    }
    if (mounted) {
      showModalBottomSheet(
        ...
        builder: (_) => SafeArea(
          child: ListView(
            children: [
              const Padding(padding: EdgeInsets.all(16), child: Text('Версии каталогов', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w800))),
              ...rows,
            ],
          ),
        ),
  ...
  Future<void> _settings() async {
    ...
    if (!mounted) return;
    showModalBottomSheet(context: context, isScrollControlled: true, builder: (_) => const _SettingsSheet());
  }
  ...
  Widget build(BuildContext context) {
    ...
    final total = _summaries.fold(0, (s, x) => s + x.total);
    final owned = _summaries.fold(0, (s, x) => s + x.owned);
    final wanted = _summaries.fold(0, (s, x) => s + x.wanted);
    final ordered = _summaries.fold(0, (s, x) => s + x.ordered);
    final cost = _summaries.fold(0.0, (s, x) => s + x.cost);
      ...
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            ...
            _Tool('Настройки интерфейса', 'Плотность, колонки и высота карточек', Icons.tune, _settings),
            _Tool('Версии каталогов', 'Проверка версии каталога', Icons.new_releases_outlined, _versions),
            Card(child: ExpansionTile(title: const Text('Резервная копия и экспорт'), leading: const Icon(Icons.backup_outlined), children: [Wrap(spacing: 8, runSpacing: 8, children: [FilledButton.tonal(onPressed: () => _export('json'), child: const Text('JSON')), FilledButton.tonal(onPressed: () => _export('csv'), child: const Text('CSV')), FilledButton.tonal(onPressed: () => _export('xlsx'), child: const Text('Excel')), FilledButton.tonal(onPressed: () => _export('pdf'), child: const Text('PDF')), FilledButton(onPressed: () => _export('zip'), child: const Text('ZIP'))]), const SizedBox(height: 12)])),
            Card(child: ExpansionTile(title: const Text('История изменений'), leading: const Icon(Icons.history), children: _history.isEmpty ? [const ListTile(title: Text('История пока пуста'))] : _history.take(30).map((e) => ListTile(title: Text(e.title), subtitle: Text('${e.details}\n${e.timestamp}'))).toList())),
            Card(child: ExpansionTile(title: const Text('Осталось собрать'), leading: const Icon(Icons.playlist_add_check), children: _summaries.map((s) => ListTile(title: Text(s.collection.name), subtitle: Text('Не хватает ${s.total - s.owned} из ${s.total}'), trailing: Text('${s.percent.round()}%'))).toList())),
          ],
        ),
      ),
    );
...
class _Summary {
  ...
  final int total, owned, wanted, ordered, quantity;
  final double cost;
...
class _Metric extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext c) => Card(child: Padding(padding: const EdgeInsets.all(10), child: Column(children: [Icon(icon), Text(value, style: const TextStyle(fontWeight: FontWeight.w900)), Text(label, textAlign: TextAlign.center, style: Theme.of(c).textTheme.bodySmall)])));
}
...
class _Tool extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext c) => Card(child: ListTile(leading: CircleAvatar(child: Icon(icon)), title: Text(title, style: const TextStyle(fontWeight: FontWeight.w800)), subtitle: Text(subtitle), trailing: const Icon(Icons.chevron_right), onTap: onTap));
}
        ...
        child: ListView(
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              ...
              onChanged: (v) => setState(() => density = v ?? 'auto'),
            ),
            const SizedBox(height: 12),
            DropdownButtonFormField<int>(
              ...
              onChanged: (v) => setState(() => columns = v ?? 0),
            ),
            const SizedBox(height: 12),
            DropdownButtonFormField<double>(
              ...
              items: const [
                DropdownMenuItem(value: 110, child: Text('Компактная — 110')),
              ...
              onChanged: (v) => setState(() => cardHeight = v ?? 150),
            ),
            const SizedBox(height: 12),
            DropdownButtonFormField<String>(
              ...
              onChanged: (v) => setState(() => navigation = v ?? 'auto'),
            ),
