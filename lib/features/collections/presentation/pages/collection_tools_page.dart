...
class _CollectionToolsPageState extends ConsumerState<CollectionToolsPage> {
  ...
  void initState() {
    ...
    _wishlistListener = () { if (mounted) _load(); };
    CurrencySettings.revision.addListener(_currencyListener!);
    WishlistStore.revision.addListener(_wishlistListener!);
    _initialize();
  }

  Future<void> _initialize() async {
    try { await CurrencySettings.load(); } finally { if (mounted) await _load(); }
  }

  @override
  void dispose() {
    final currencyListener = _currencyListener;
    final wishlistListener = _wishlistListener;
    if (currencyListener != null) CurrencySettings.revision.removeListener(currencyListener);
    if (wishlistListener != null) WishlistStore.revision.removeListener(wishlistListener);
    super.dispose();
  }
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final wishlistIds = await WishlistStore.loadIds();
      final summaries = await Future.wait(collections.map((collection) async {
        final items = await service.getItems(collection.id);
        var owned = 0, wanted = 0, ordered = 0, quantity = 0;
        var cost = 0.0;
      ...
      if (!mounted) return;
      setState(() { _summaries = summaries; _history = history; _wishlistCount = wishlistIds.length; _loading = false; });
    } catch (error) {
      if (!mounted) return;
    ...
    try {
      ...
      for (final collection in collections) {
        ...
        for (final item in items) {
          ...
          final owned = state?.status == CollectionItemStatus.owned || state?.status == CollectionItemStatus.storage;
          if (!owned) ids.add(item.id);
        }
      }
      await WishlistStore.addAll(ids);
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('В Wishlist добавлено отсутствующих: ${ids.length}')));
      await _load();
    } catch (error) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Не удалось сформировать Wishlist: $error')));
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_error != null) return Scaffold(appBar: AppBar(title: const Text('Обзор')), body: Center(child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.error_outline, size: 48), const SizedBox(height: 12), const Text('Не удалось загрузить обзор', textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.w700)), const SizedBox(height: 8), Text(_error!, textAlign: TextAlign.center), const SizedBox(height: 16), FilledButton.icon(onPressed: _load, icon: const Icon(Icons.refresh), label: const Text('Повторить'))])));
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(onRefresh: _load, child: ListView(padding: const EdgeInsets.all(16), children: [
        ...
        Card(child: ListTile(leading: const CircleAvatar(child: Icon(Icons.favorite_border)), title: Text('Wishlist · $_wishlistCount'), subtitle: const Text('Все отсутствующие предметы, которые вы хотите собрать'), trailing: const Icon(Icons.chevron_right), onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const WishlistPage())).then((_) => _load()))),
        OutlinedButton.icon(onPressed: _addAllMissing, icon: const Icon(Icons.playlist_add), label: const Text('Добавить всё отсутствующее')),
