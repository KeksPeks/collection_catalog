...
class _CollectionToolsPageState extends ConsumerState<CollectionToolsPage> {
  ...
  void dispose() {
    ...
    if (currencyListener != null) {
      CurrencySettings.revision.removeListener(currencyListener);
    }
    if (wishlistListener != null) {
      WishlistStore.revision.removeListener(wishlistListener);
    }
    super.dispose();
  }
    ...
    try {
      ...
      final summaries = await Future.wait(collections.map((collection) async {
        ...
        for (final item in items) {
          ...
          if (state.status == CollectionItemStatus.owned ||
              state.status == CollectionItemStatus.storage) {
            owned++;
          }
    ...
    try {
      ...
      for (final collection in collections) {
        ...
        for (final item in items) {
          ...
          final owned = state?.status == CollectionItemStatus.owned ||
              state?.status == CollectionItemStatus.storage;
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('В Wishlist добавлено отсутствующих: ${ids.length}'),
          ),
        );
      }
      await _load();
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Не удалось сформировать Wishlist: $error')),
        );
      }
    }
  }
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              ok ? 'Уведомления разрешены.' : 'Разрешение не выдано.',
            ),
          ),
        );
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Уведомления недоступны: $error')),
        );
      }
    }
  }
      ...
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(child: _Metric('Всего', '$total', Icons.inventory_2_outlined)),
                const SizedBox(width: 8),
                Expanded(child: _Metric('Есть', '$owned', Icons.check_circle_outline)),
                const SizedBox(width: 8),
                Expanded(child: _Metric('Не хватает', '${total - owned}', Icons.remove_circle_outline)),
              ],
            ),
            const SizedBox(height: 8),
            Row(
              children: [
                Expanded(child: _Metric('Хочу', '$wanted', Icons.shopping_cart_outlined)),
                const SizedBox(width: 8),
                Expanded(child: _Metric('Заказано', '$ordered', Icons.local_shipping_outlined)),
                const SizedBox(width: 8),
                Expanded(child: _Metric('Расходы', '$currency${cost.toStringAsFixed(2)}', Icons.currency_exchange_rounded)),
              ],
            ),
            const SizedBox(height: 12),
            Card(
              child: ListTile(
                ...
                title: Text('Wishlist · $_wishlistCount'),
                subtitle: const Text('Все отсутствующие предметы, которые вы хотите собрать'),
            ...
            OutlinedButton.icon(
              onPressed: _addAllMissing,
              icon: const Icon(Icons.playlist_add),
              label: const Text('Добавить всё отсутствующее'),
            ),
            _Tool(
              'Достижения',
              'XP, уровни и прогресс коллекционера',
              Icons.emoji_events_outlined,
              () => Navigator.push(context, MaterialPageRoute(builder: (_) => const GamificationPage())),
            ),
            _Tool(
              'Глобальный поиск',
              'По каталогам, сериям и предметам',
              Icons.search,
              () => Navigator.push(context, MaterialPageRoute(builder: (_) => const GlobalSearchPage())),
            ),
            _Tool(
              'QR / штрихкод',
              'Сканировать код предмета',
              Icons.qr_code_scanner,
              () => Navigator.push(context, MaterialPageRoute(builder: (_) => const BarcodeScannerPage())),
            ),
            ...
            Card(
              child: ExpansionTile(
                ...
                children: _history.isEmpty
                    ? [const ListTile(title: Text('История пока пуста'))]
                    : _history.take(30).map((entry) {
                        return ListTile(
            ...
            Card(
              child: ExpansionTile(
                ...
                children: _summaries.map((summary) {
                  return ListTile(
                    title: Text(summary.collection.name),
                    subtitle: Text('Не хватает ${summary.total - summary.owned} из ${summary.total}'),
                    trailing: Text('${summary.percent.round()}%'),
                  );
                }).toList(),
              ),
            ),
