          ...
          children: [
            Card(child: ListTile(
              ...
              subtitle: collectionAsync.when(
                data: (collection) => Text('Централизованный каталог · ${collection?.templateId ?? 'каталог'}'),
            ...
            FilledButton.icon(
              onPressed: () async {
                final collection = collectionAsync.valueOrNull ?? Collection(id: collectionId, name: collectionName, createdAt: DateTime.now(), updatedAt: DateTime.now());
            ...
            _SectionCard(
              ...
              child: itemsAsync.when(
                ...
                data: (items) => fieldsAsync.when(
                  ...
                  error: (_, __) => Text('Физических экземпляров: ${items.length}'),
                  data: (fields) => Text('Физических экземпляров: ${items.length} · Полей: ${fields.length}'),
                ),
              ),
            ),
            const SizedBox(height: 12),
            _SectionCard(
              ...
              child: itemsAsync.when(
                ...
                data: (items) => items.isEmpty
                    ...
                    : Column(children: items.take(20).map((item) {
                        final linked = item.catalogItemId != null;
                        return ListTile(
                          contentPadding: EdgeInsets.zero,
                          leading: Icon(linked ? Icons.copy_all_outlined : Icons.inventory_2_outlined),
                          title: Text(linked ? 'Каталожная позиция ${item.catalogItemId}' : item.id),
                          subtitle: linked ? Text('Физический экземпляр: ${item.id}') : const Text('Физический экземпляр'),
                          trailing: const Icon(Icons.chevron_right),
                          onTap: () {
                            if (item.catalogItemId != null) {
                              Navigator.of(context).push(MaterialPageRoute(builder: (_) => CatalogItemDetailPage(
                                catalogItemId: item.catalogItemId!,
                                collectionId: item.collectionId,
                                title: 'Каталожная позиция ${item.catalogItemId}',
                              )));
                            } else {
                              Navigator.of(context).push(MaterialPageRoute(builder: (_) => ItemDetailPage(itemId: item.id)));
                            }
                          },
                        );
                      }).toList(),
            ...
            _SectionCard(
              ...
              child: fieldsAsync.when(
                ...
                data: (fields) => fields.isEmpty
                    ...
                    : Column(children: fields.map((field) => ListTile(
                        contentPadding: EdgeInsets.zero,
