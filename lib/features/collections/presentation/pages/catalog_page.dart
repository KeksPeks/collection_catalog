...
class _CatalogPageState extends ConsumerState<CatalogPage> {
      ...
      appBar: AppBar(
        ...
        actions: [
          PopupMenuButton<String>(
            ...
            itemBuilder: (_) => [for (final option in CurrencySettings.options) CheckedPopupMenuItem<String>(value: option.code, checked: option.code == CurrencySettings.code, child: Text('${option.symbol} ${option.name(Localizations.localeOf(context))}'))],
          ),
        ...
        child: ListView(
          ...
          children: [
            ...
            Row(children: [
              ...
              SegmentedButton<bool>(segments: const [ButtonSegment(value: false, icon: Icon(Icons.view_list_rounded)), ButtonSegment(value: true, icon: Icon(Icons.grid_view_rounded))], selected: {_grid}, showSelectedIcon: false, onSelectionChanged: (value) => _setLayout(value.first)),
            ]),
            const SizedBox(height: 10),
            if (categories.isEmpty) _buildEmpty(context, l10n) else _buildCategories(context, categories, l10n, colors),
          ],
        ),
      ),
    );
  ...
  Widget _buildCategories(BuildContext context, List<CatalogCategoryDefinition> categories, AppLocalizations l10n, ColorScheme colors) {
    return LayoutBuilder(
      builder: (context, constraints) {
        final columns = _grid ? UiLayoutSettings.resolveColumns(constraints.maxWidth) : 1;
        final cardHeight = UiLayoutSettings.resolveCardHeight(width: constraints.maxWidth, columns: columns, textScale: MediaQuery.textScalerOf(context).scale(1));
        if (!_grid) {
          return Column(
  ...
  Widget _buildFavorites(BuildContext context, List<CatalogDefinition> catalogs) {
    return LayoutBuilder(
      builder: (context, constraints) {
        final height = UiLayoutSettings.resolveCardHeight(width: constraints.maxWidth, columns: 1, textScale: MediaQuery.textScalerOf(context).scale(1));
        return SizedBox(
          height: height,
          child: ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              return SizedBox(width: 280, child: _FavoriteCard(catalog: catalog, title: CatalogUiLocalization.catalogName(context, catalog.id), onTap: () => _openCatalog(catalog), onToggle: () => _toggleFavorite(catalog.id)));
            },
          ),
        );
      },
...
class _CategoryListTile extends StatelessWidget {
  ...
  const _CategoryListTile({required this.category, required this.title, required this.subtitle, required this.height, required this.onTap});
  @override
  Widget build(BuildContext context) {
    final colors = Theme.of(context).colorScheme;
    return SizedBox(
        ...
        child: InkWell(
          ...
          child: Padding(
            ...
            child: Row(children: [
              ...
              Expanded(child: Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w800)), const SizedBox(height: 4), Text('${catalog.entries.length} записей')])),
