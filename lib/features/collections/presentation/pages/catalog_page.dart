...
class _CatalogPageState extends ConsumerState<CatalogPage> {
                  ...
                  itemBuilder: (_) => [
                    for (final option in CurrencySettings.options)
                      CheckedPopupMenuItem<String>(
                        value: option.code,
                        checked: _currencyLoaded && option.code == CurrencySettings.code,
            ...
            SliverPadding(
              ...
              sliver: SliverList(
                delegate: SliverChildListDelegate([
                  ...
                  if (favorites.isEmpty) _buildNoFavorites(context, l10n) else _buildFavorites(context, favorites),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text('Валюта: ${CurrencySettings.label(context)}', style: Theme.of(context).textTheme.labelLarge?.copyWith(color: colors.primary, fontWeight: FontWeight.w800)),
              ],
            ),
          ),
  ...
  Widget _buildCategories(BuildContext context, List<CatalogCategoryDefinition> categories, AppLocalizations l10n) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        final columns = constraints.maxWidth >= 1100 ? 4 : constraints.maxWidth >= 750 ? 3 : constraints.maxWidth >= 500 ? 2 : 1;
        return GridView.builder(
          shrinkWrap: true,
          physics: const NeverScrollableScrollPhysics(),
          itemCount: categories.length,
          gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: columns, crossAxisSpacing: 12, mainAxisSpacing: 12, mainAxisExtent: 210),
  ...
  Widget _buildNoFavorites(BuildContext context, AppLocalizations l10n) {
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        const SizedBox(height: 10),
        Text('${l10n.noFavorites}. ${l10n.noFavoritesDescription}', textAlign: TextAlign.center),
      ]),
    );
  }
  ...
  List<CatalogCategoryDefinition> _filteredCategories(BuildContext context) {
    ...
    return CatalogRegistry.categories.where((category) {
      final categoryName = CatalogUiLocalization.categoryName(context, category.id).toLowerCase();
      return categoryName.contains(query) || category.catalogIds.any((id) => CatalogUiLocalization.catalogName(context, id).toLowerCase().contains(query));
    }).toList(growable: false);
  }
  ...
  Future<void> _showSearch(BuildContext context) async {
    ...
    await showDialog<void>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: TextField(controller: controller, autofocus: true, decoration: InputDecoration(hintText: AppLocalizations.of(context).searchHint), onChanged: (value) => setState(() => _search = value)),
...
class _CategoryListTile extends StatelessWidget {
  final CatalogCategoryDefinition category; final String title; final String subtitle; final VoidCallback onTap;
  ...
  IconData _icon(String id){switch(id){case 'constructors':return Icons.extension_rounded;case 'numismatics':return Icons.monetization_on_outlined;case 'banknotes':return Icons.payments_outlined;case 'cards':return Icons.style_outlined;case 'video_games':return Icons.sports_esports_outlined;case 'consoles':return Icons.gamepad_outlined;case 'movies':return Icons.movie_outlined;case 'figurines':return Icons.toys_outlined;case 'books':return Icons.menu_book_outlined;case 'music':return Icons.album_outlined;case 'sports':return Icons.emoji_events_outlined;case 'art':return Icons.palette_outlined;case 'antiques':return Icons.history_edu_outlined;default:return Icons.category_outlined;}}
}
