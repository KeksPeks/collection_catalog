...
class _CatalogPageState extends ConsumerState<CatalogPage> {
  ...
  void dispose() {
    ...
    if (favoriteListener != null) {
      FavoritesStore.revision.removeListener(favoriteListener);
    }
    final layoutListener = _layoutListener;
    if (layoutListener != null) {
      UiLayoutSettings.revision.removeListener(layoutListener);
    }
    super.dispose();
  }
            ...
            itemBuilder: (_) => [
              for (final option in CurrencySettings.options)
                CheckedPopupMenuItem<String>(
                  ...
                  child: Text(
                    '${option.symbol} ${option.name(Localizations.localeOf(context))}',
                  ),
                ),
            ],
          ),
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final showText = columns == 1 || categories.every((category) {
          ...
          )..layout(maxWidth: columnWidth - 28);
          return !painter.didExceedMaxLines;
        });
        final height = showText
            ? (118 * textScale).clamp(104.0, 170.0).toDouble()
            : 82.0;
  ...
  Widget _buildFavorites(
    ...
    final textScale = MediaQuery.textScalerOf(context).scale(1.0);
    return SizedBox(
      height: (96 * textScale).clamp(88.0, 126.0).toDouble(),
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          ...
          final width = (painter.width + 94).clamp(150.0, 260.0).toDouble();
          return SizedBox(
            width: width,
            child: _FavoriteCard(
              catalog: catalog,
  ...
  List<CatalogCategoryDefinition> _filteredCategories(BuildContext context) {
    ...
    return CatalogRegistry.categories.where((category) {
      final name =
          CatalogUiLocalization.categoryName(context, category.id).toLowerCase();
      return name.contains(query) ||
          category.catalogIds.any(
            (id) => CatalogUiLocalization.catalogName(context, id)
                .toLowerCase()
                .contains(query),
          );
    }).toList(growable: false);
  ...
  ) => path.isEmpty ? null : sectionIds[path.join('/')];
        ...
        child: Padding(
          ...
          child: Row(
            children: [
              CircleAvatar(
                radius: 20,
                child: Icon(_icon(catalog.id), size: 20),
              ),
              const SizedBox(width: 8),
              Expanded(
                child: Text(
