  ...
  Future<void> _load() async {
    ...
    if (sectionIds.isNotEmpty || itemIds.isNotEmpty) {
      ...
      for (final collection in collections) {
        final sections = sectionIds.isEmpty
            ? const <dynamic>[]
            : await sectionService.getSections(collection.id);
        ...
        final items = itemIds.isEmpty
            ? const <dynamic>[]
            : await itemService.getItems(collection.id);
              ...
              children: [
                ...
                if (items.isNotEmpty) ...[
                  ...
                  for (final id in items)
                    _Tile(
                      _labels[id] ?? id.substring(5),
                      'Предмет коллекции',
                      Icons.inventory_2_outlined,
                      () => Navigator.of(context).push(
                        MaterialPageRoute(
                          builder: (_) =>
                              ItemDetailPage(itemId: id.substring(5)),
                        ),
                      ),
                      () => FavoritesStore.removeKey(id),
                    ),
