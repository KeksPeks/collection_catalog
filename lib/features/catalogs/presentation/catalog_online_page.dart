...
class _CatalogOnlinePageState extends State<CatalogOnlinePage> {
  ...
  bool get _isCoinGroup =>
      widget.sectionPath.isNotEmpty &&
      widget.sectionPath.first == '__coin_group__';

  bool get _regularCoins =>
      widget.catalog.id == 'coins' &&
      widget.sectionPath.join('/') == 'countries/russia/regular';

  CatalogSectionDefinition? get _currentSection {
    if (widget.sectionPath.isEmpty || _isCoinGroup) return null;
  ...
  List<CatalogSectionDefinition> get _sections {
    if (_isCoinGroup) return const <CatalogSectionDefinition>[];

    final source = widget.sectionPath.isEmpty
        ? widget.catalog.sections
        : (_currentSection?.children ??
            const <CatalogSectionDefinition>[]);

    // Для нумизматики «Страны» — техническая папка. Сразу показываем страны.
    if (widget.catalog.id == 'coins' && widget.sectionPath.isEmpty) {
      final countries = source
          .where((section) => _isCountriesSection(section))
          .firstOrNull;
      if (countries != null && countries.children.isNotEmpty) {
  ...
  List<CatalogEntryDefinition> get _entries {
    ...
    if (_isCoinGroup) {
      final field = widget.sectionPath.length > 1
          ? widget.sectionPath[1]
          : 'denomination';
      final group = widget.sectionPath.length > 2
          ? widget.sectionPath[2]
          : '';
      return widget.catalog.entries
          .where(
            (entry) =>
                _coinAttribute(entry, field) == group &&
                _matchesPath(
                  entry,
                  const ['countries', 'russia', 'regular'],
                ),
          )
    ...
    return widget.catalog.entries
        .where((entry) => _matchesPath(entry, widget.sectionPath))
        .toList(growable: false);
  }
  ...
  String _coinAttribute(CatalogEntryDefinition entry, String field) {
    ...
    for (final candidate in candidates) {
      final value = entry.attributes.entries
          .where(
            (item) =>
                item.key.trim().toLowerCase() == candidate.toLowerCase(),
          )
          .map((item) => item.value)
          .firstOrNull;
  ...
  int _naturalCompare(String a, String b) {
    final an = double.tryParse(
      a.replaceAll(',', '.').replaceAll(RegExp(r'[^0-9.-]'), ''),
    );
    final bn = double.tryParse(
      b.replaceAll(',', '.').replaceAll(RegExp(r'[^0-9.-]'), ''),
    );
  ...
  List<_EntryGroup> get _groups {
    if (!_regularCoins) return const <_EntryGroup>[];

    final groups = <String, List<CatalogEntryDefinition>>{};
    for (final entry in _entries) {
      final key = _coinAttribute(entry, _coinGroupField);
      groups
          .putIfAbsent(key.isEmpty ? 'Без категории' : key, () => <CatalogEntryDefinition>[])
          .add(entry);
    }

    final result = groups.entries
        .map((item) => _EntryGroup(name: item.key, entries: item.value))
        .toList();
  ...
  void _selectCoinGroup(String field) {
    setState(() => _coinGroupField = field);
  }

  @override
  Widget build(BuildContext context) {
    final l10n = AppLocalizations.of(context);
    final title = _isCoinGroup
        ? widget.sectionPath.last
        : _currentSection?.name ?? l10n.catalogName(widget.catalog.id);
    final sections = _sections;
    final groups = _groups;
    final entries = _entries;
    final showEntries = !_regularCoins &&
        ((widget.sectionPath.isNotEmpty && sections.isEmpty) ||
            (widget.sectionPath.isEmpty && widget.catalog.sections.isEmpty));
    final showEmpty = sections.isEmpty &&
        groups.isEmpty &&
        !showEntries &&
        !_regularCoins;
          ...
          if (_regularCoins)
            PopupMenuButton<String>(
              ...
              itemBuilder: (context) => const [
                PopupMenuItem(value: 'denomination', child: Text('По номиналу')),
                PopupMenuItem(value: 'year', child: Text('По году выпуска')),
                PopupMenuItem(value: 'series', child: Text('По серии')),
              ],
            ),
        ],
      ),
        ...
        itemBuilder: (context, index) {
          if (index < sections.length) {
            ...
            return _SectionCard(
              ...
              onTap: () => _pushPath([...widget.sectionPath, section.id]),
            );
          }
          ...
          if (groupIndex < groups.length) {
            ...
            return _GroupCard(
              group: group,
              onTap: () => _pushCoinGroup(_coinGroupField, group.name),
            );
          }
          ...
          if (showEntries && entryIndex < entries.length) {
            ...
            return _CatalogEntryCard(
              entry: entry,
              onTap: () => Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (_) => CatalogEntryDetailPage(
  ...
  void _pushPath(List<String> path) {
  ...
  void _pushCoinGroup(String field, String value) {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => CatalogOnlinePage(
          catalog: widget.catalog,
          onDownload: widget.onDownload,
          sectionPath: ['__coin_group__', field, value],
        ),
      ),
    );
  }
    ...
    return Card(
      ...
      child: InkWell(
        ...
        child: ListTile(
          ...
          subtitle: Text('${group.entries.length} записей'),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (section.children.isNotEmpty)
                      Text('${section.children.length} подразделов'),
                  ],
                ),
              ),
              IconButton(
      ...
      child: InkWell(
        ...
        child: Padding(
          ...
          child: Row(
            ...
            children: [
              _CatalogVisual(
                label: entry.attributes['Страна'] ?? entry.primaryValue,
      ...
      child: ClipRRect(
        ...
        child: flag != null
            ? ColoredBox(
                color: surface,
                child: Center(
                  child: Text(flag, style: const TextStyle(fontSize: 42)),
                ),
              )
