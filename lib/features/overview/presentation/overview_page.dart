...
/// Локальный обзор коллекционера.
/// Все расчёты выполняются на устройстве и не требуют сервера.
...
class _OverviewBody extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return FutureBuilder<_Stats>(
      ...
      builder: (context, snapshot) {
        ...
        final achievements = _achievements(stats);
  ...
  Future<_Stats> _loadStats(WidgetRef ref, List<dynamic> collections) async {
    ...
    return _Stats(collections.length, items);
  }
...
class _StatGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) => LayoutBuilder(builder: (context, constraints) {
    ...
    return GridView.count(crossAxisCount: columns, shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), mainAxisSpacing: 10, crossAxisSpacing: 10, childAspectRatio: columns == 1 ? 3.2 : 2.4, children: [_StatCard(Icons.collections_bookmark_outlined, 'Коллекции', stats.collections), _StatCard(Icons.inventory_2_outlined, 'Предметы', stats.items)]);
  });
}
