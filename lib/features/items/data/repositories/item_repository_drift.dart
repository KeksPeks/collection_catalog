...
class ItemRepositoryDrift implements ItemRepository {
  ...
  Future<List<Item>> getItemsByCatalogItem(String catalogItemId) async {
    final rows = await database.customSelect(
      '''
      SELECT i.id, i.collection_id, i.section_id, i.sort_order,
  ...
  Future<Item> _fromRow(ItemTableData row) async {
    final rows = await database.customSelect(
      'SELECT catalog_item_id FROM item_catalog_links WHERE item_id = ? LIMIT 1',
      variables: [Variable.withString(row.id)],
    ).get();

    return Item(
      id: row.id,
      catalogItemId: rows.isEmpty
          ? null
