...
final catalogItemInstancesProvider = FutureProvider.family<List<Item>, String>(
  (ref, catalogItemId) => ref.watch(itemServiceProvider).getItemsByCatalogItem(catalogItemId),
);
...
final collectionItemValuesProvider = FutureProvider.family<Map<String, Map<String, String>>, String>(
  (ref, collectionId) async {
    ...
    for (final item in items) {
      ...
      result[item.id] = {for (final value in values) value.fieldId: value.value};
    }
    return result;
  },
);
