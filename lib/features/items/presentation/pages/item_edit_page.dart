...
class _ItemEditPageState extends ConsumerState<ItemEditPage> {
  ...
  void _initializeControllers(List<FieldDefinition> fields, List<ItemValue> values) {
    if (_initialized) return;
    final valuesByField = <String, ItemValue>{for (final value in values) value.fieldId: value};
    for (final field in fields) {
      final existing = valuesByField[field.id];
      _controllers[field.id] = TextEditingController(text: existing?.value ?? '');
      if (existing != null) _valueIds[field.id] = existing.id;
    }
    _initialized = true;
  }
  ...
  Future<void> _save(List<FieldDefinition> fields) async {
    ...
    for (final field in fields) {
      ...
      if (existingId != null) {
        ...
        await service.updateValue(ItemValue(id: existingId, itemId: widget.item.id, fieldId: field.id, value: value));
      } else if (value.isNotEmpty) {
        final id = DateTime.now().microsecondsSinceEpoch.toString();
        await service.saveValue(ItemValue(id: id, itemId: widget.item.id, fieldId: field.id, value: value));
  ...
  Widget build(BuildContext context) {
    final fieldsAsync = ref.watch(fieldsProvider(widget.item.collectionId));
    final valuesAsync = ref.watch(itemValuesProvider(widget.item.id));

    return Scaffold(
      appBar: AppBar(title: const Text('Изменить предмет')),
          ...
          data: (values) {
            ...
            return ListView(
              ...
              children: [
                ...
                FilledButton(onPressed: () => _save(fields), child: const Text('Сохранить')),
              ],
            );
          },
        ),
